...
def generate_initial_population(population_size, num_gene,num_job):
...
def job_schedule(data_dict, population_size = 30, crossover_rate = 0.7, mutation_rate = 0.3, mutation_selection_rate = 0.3, num_iteration = 2000):
    ...
    population_list = generate_initial_population(population_size,num_gene,num_job)
    makespan_record = []
    ...
    """ Results - Makespan """
    print(f'Optimal value is {Tbest} for Population size {population_size} and Number of  Iteration {num_iteration}')
    return  Tbest

data = data_excel_json('JSP_dataset.xlsx')
# NOTE: Check here for all permutation of population_size,num_iteration
row_pop = 2
for pop in range(10,70,10):
    for iter in range(500,6000,500):
        c1 = sheet.cell(row=row_pop,column=1)
        c1.value=pop
        c2 = sheet.cell(row=row_pop,column=2)
        c2.value=iter
        makespan = job_schedule(data_dict=data,population_size=pop,num_iteration=iter)
